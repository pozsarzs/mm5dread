@echo off
rem ----------------------------------------------------------------------------
rem  MM5DRead v0.3 * Status reader program for MM5D device
rem  Copyright (C) 2020-2023 Pozsár Zsolt <pozsar.zsolt@szerafingomba.hu>
rem  build.bat
...
echo.
%PPC% %FPFLAG1% %FPFLAG2% %FPFLAG3% %FPFLAG4% mm5dread.lpr
echo.
...
if not exist "source\lib\%ARCH%-%OS%\mm5dread.exe" ( echo Error: firstly run "build.bat" to compile source code! & goto end )
...
config\mkshortcut.vbs /target:"%INSTDIR%\%NAME%.exe" /shortcut:"%USERPROFILE%\Desktop\MM5DRead"
echo.
...
rd /s "%INSTDIR%"
del /q "%USERPROFILE%\Desktop\MM5DRead.lnk"
goto end
